    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                logging.info("Quit event detected during race simulation. Exiting.")
                return None, "end", is_muted
...
def simulate_all_characters_race(screen, sprotos, race_distance, race_duration, race_backgrounds, single_bg_options, trophy_image, is_muted=False):
    ...
    if not sprotos:  # Handle case where no racers are provided
        logging.info("No racers provided for the race. Displaying empty race screen.")
        render_race_screen(
            screen,
            sprotos=[],
            ...
            race_number=None,
            winner=None,
            track_surface=cache_track_surface(race_distance, 0),
            buttons=[],
            current_background=random.choice(single_bg_options),
            flash_timer=0,
            is_muted=is_muted,
            lane_height=100
        )
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                logging.info("Quit event detected during all-characters race simulation. Exiting.")
                return None, is_muted
        ...
        if not race_finished:
            ...
            if all(sproto.finished for sproto in sprotos):
                race_finished = True
                winner = finishers[0] if finishers else max(sprotos, key=lambda h: h.position)
                logging.info(f"Race finished! Winner: {winner.name}")

        render_race_screen(screen, sprotos, race_distance, time_elapsed, None, winner, track_surface, [], current_background, flash_timer, is_muted, lane_height=lane_height)
        pygame.display.flip()

        if race_finished:
            pygame.time.wait(3000)  # Wait 3 seconds before returning
            running = False

    return winner, is_muted